...
```
Depending on OS (eg. Ubuntu Server 22.04) installing Broadcom libraries may be also required:
```
sudo apt-get install libraspberrypi-dev
```  
After installing dependencies clone this repository and use `make` command in order to build executable:
```
git clone https://github.com/Tarunesh38/Fm_Radio-Transmitter.git
cd fm_transmitter
make
```
After a successful build you can start transmitting by executing the "fm_transmitter" program:
```
sudo ./fm_transmitter -f 106.2 Avengers Endgame.wav
```
Notice:
* -f frequency - Specifies the frequency in MHz, 100.0 by default if not passed
*Avengers Endgame.wav - Sample WAV file, you can use your own
...
```
arecord -D hw:1,0 -c 1 -d 0 -r 22050 -f S16_LE | sudo ./fm_transmitter -f 106.2 -
```
...
sox Avengers Endgame.mp3 -r 22050 -c 1 -b 16 -t wav - | sudo ./fm_transmitter -f 106.2 -
```
...
```
sudo apt-get install sox libsox-fmt-mp3
sox Avengers Endgame.mp3 -r 22050 -c 1 -b 16 -t wav converted-example.wav
...
sudo ./fm_transmitter -f 106.2 converted-example.wav
```
## Legal note
